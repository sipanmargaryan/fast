...
def test_users_vector_success(monkeypatch):
    ...
    response = client.post('/api/v1/users/update-user-vector', data=json.dumps(data))
...
def test_users_vector_error():
    data = {"user_id": 24, "content_id": 24, "update_type": "post"}
    response = client.post('/api/v1/users/update-user-vecto', data=json.dumps(data))
...
def test_generate_user_vector():
    ...
    response = client.post('/api/v1/users/generate-user-vector', data=json.dumps(data))
    assert response.status_code == 200
    assert len(response.json()["vector"]) == 220
